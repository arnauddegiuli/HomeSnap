...
public class OpenWebConnectThread implements Runnable {
	...
	public OpenWebConnectThread(OpenWebCommanderImpl commander, String command, CommandListener resultListener) {
	...
	public void run() {
		if (connect() && command != null) {
			commander.sendCommand(command, resultListener);
		}
		stop();
	}
