...
import com.adgsoftware.mydomo.engine.actuator.connector.ConnectionListener;
import com.adgsoftware.mydomo.engine.actuator.connector.ConnectionStatusEnum;
import com.adgsoftware.mydomo.engine.actuator.connector.Monitor;
import com.adgsoftware.mydomo.engine.actuator.connector.UnknownControllerListener;
...
implements Monitor {
	...
	protected void onMessageReceipt(String message) {
		try {
			...
			com.adgsoftware.mydomo.engine.actuator.what.core.State what = new com.adgsoftware.mydomo.engine.actuator.what.core.State(StateName.STATUS, new StringValue(parser.getWhatFromCommand())); // TODO mapping WHAT
			Who who = Who.valueOf(parser.getWhoFromCommand()); // TODO mapping WHO
			boolean known = false;
	...
	private boolean updateController(Who who, com.adgsoftware.mydomo.engine.actuator.what.core.State what, Where where, String message, Command parser) {
		boolean known = false;
		if (what != null) {
			// Manage what command
			for (Controller controller : controllerList) {
